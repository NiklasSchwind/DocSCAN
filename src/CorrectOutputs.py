...
def correct_accuracy_calculation_selflabeling(in_file, out_file=None):
    ...
    for j, line in enumerate(lines_file):
        if line[0:3] == '[0.':
            ...
            weighted_accuracy_afterSL.append(float(lines_file[j-11].split(', ')[0].split(' ')[3]))
            out.write(line)
...
correct_folder('/vol/fob-vol7/mi19/schwindn/DocSCAN/DeletionRatioExperimentsNew', '/vol/fob-vol7/mi19/schwindn/DocSCAN/DeletionRatioExperimentsTwiceCorrected')
