...
class DocSCANPipeline():
	...
	def run_main(self):
		...
		for _ in range(10):
			...
			print(torch.max(torch.max(torch.nn.Softmax(dim=0)(torch.tensor(probabilities)),1)))
			self.df_test["probabilities"] = torch.nn.Softmax(dim=0)(torch.tensor(probabilities))
