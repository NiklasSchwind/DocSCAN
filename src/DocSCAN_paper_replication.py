...
class DocSCANPipeline():
	...
	def embedd_sentences_method(self, sentences, method, loadpath = None):
		if method == 'SBert':
			...
			#TODO find way to include Universal sentence encoder
	...
	def run_main(self):
		...
		if os.path.exists(os.path.join(self.args.path, "embeddings.npy")):
			...
			self.embeddings = self.embedd_sentences_method(df_train["sentence"], 'TSDAE') #self.embeddings = self.embedd_sentences(df_train["sentence"])
		...
		if os.path.exists(os.path.join(self.args.path, "embeddings_test.npy")):
			...
			self.embeddings_test = self.embedd_sentences_method(self.df_test["sentence"], 'TSDAE')# self.embedd_sentences(self.df_test["sentence"])
