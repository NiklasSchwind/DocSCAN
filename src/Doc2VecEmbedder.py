...
class Doc2Vec_Embedder:
    ...
    def TrainDoc2Vec(self):
        ...
        model_dbow = Doc2Vec(dm=0, vector_size=768, negative=5, hs=0, min_count=2, sample=0, workers= self.cores, dbow_words = 1)
    ...
    def embed(self, data: List[str]):
        ...
        embeddings = [self.model.infer_vector(doc.words) for doc in data_tagged ] #self.model.infer_vector(doc.words, steps=20)
...
def create_embeddings(dataset_folder: str):
    ...
    np.save(os.path.join(dataset_folder, f"train-Doc2Vec768-embeddings.npy"), embeddings_train)
    np.save(os.path.join(dataset_folder, f"test-Doc2Vec768-embeddings.npy"), embeddings_test)
