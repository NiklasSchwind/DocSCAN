...
def return_list_of_accuracies_selflabeling(path):
    ...
    return pd.DataFrame(columns,
                      columns=['Dataset','Embedding','Clustering Method', 'Epochs', 'Threshold', 'Entropy Weight', 'Ratio', 'Before Selflabeling', 'After Selflabeling', 'Difference']#'Augmentation Method', 'Before Selflabeling', 'After Selflabeling', 'Difference'],
                      )
...
def display_selflabeling_experiments():
    ...
    mypath = '/vol/fob-vol7/mi19/schwindn/DocSCAN/BacktranslationExperiments'
...
def display_selflabeling_experiments_list_of_accuracy():
    ...
    frame = frame[frame.Difference != 'Experiment not finished'].sort_values(
        ['Dataset', 'Entropy Weight', 'Threshold'])  # .sort_values(['Augmentation Method','Dataset', 'Embedding', 'Clustering Method', 'Difference'])
        ...
        for i in frame['Clustering Method'].unique():
            for k in frame['Entropy Weight'].unique():
                for l in frame['Threshold'].unique():
                    print(f'{j}, {i}, {k}, {l}')
                    right_frame =  frame[(frame.Difference != 'Experiment not finished') & (frame.Dataset == j) & (frame['Clustering Method'] == i) & (frame['Entropy Weight'] == k) & (frame['Threshold'] == l)]
...
display_selflabeling_experiments_list_of_accuracy()
