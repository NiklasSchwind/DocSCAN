...
class Embedder:
    ...
    def safe_embeddings(self,mode: Literal['test', 'train','embed']):
        if self.embedding_method == 'IndicativeSentence':
            ...
            np.save(os.path.join(self.path, f"{mode}-{self.embedding_method}-embeddings_{ind.replace('<','^').replace('>','?').replace(' ','_').replace('!', '5').replace('.', '6')}.npy"), self.embeddings.cpu())
        else:
    ...
    def embed(self,
        ...
        if self.embedding_method == 'IndicativeSentence':
            ind = self.indicative_sentence
            suffix = f"_{ind.replace('<','^').replace('>','?').replace(' ','_').replace('!', '5').replace('.', '6')}"
        else:
            suffix = ''
